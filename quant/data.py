...
def get_open_thoughts(
    ...
    if num_calibration_samples:
        train_dataset_raw = train_dataset_raw.shuffle(seed=seed).select(
    ...
    train_dataset = train_dataset_raw.map(
        tokenize, remove_columns=train_dataset_raw.column_names
    )
    train_dataset = [
        torch.tensor(sample["input_ids"]).unsqueeze(0) for sample in train_dataset
    ]
...
def get_data(
    ...
    if dataset_name == "open-thoughts":
        return get_open_thoughts(
            tokenizer, max_sequence_length, num_calibration_samples, seed
        )
    if dataset_name == "open-platypus":
        return get_open_platypus(
            tokenizer, max_sequence_length, num_calibration_samples, seed
        )
    if dataset_name == "ultrachat-200k":
        return get_ultrachat_200k(
            tokenizer, max_sequence_length, num_calibration_samples, seed
        )
