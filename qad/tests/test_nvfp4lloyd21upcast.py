"""Gates for nvfp4lloyd21upcast: prefill NVFP4 derived FROM the Lloyd21 decode weight.

    shared:  W -> NVFP4                 W -> Lloyd21
    upcast:  W -> Lloyd21 -> NVFP4      W -> Lloyd21
...
silently break that and each gets an assertion rather than an assumption:
  ...
  1. The upcast could be a NO-OP. If `quantize_weight` still read the master, this
     format would be byte-identical to nvfp4lloyd21shared and every result would look
  ...
  2. STALE ORDERING. The base refreshes _wq (prefill) before _wq_dec (decode), so an
     un-overridden post_update would upcast LAST step's Lloyd21 weight forever. The
  ...
  3. The prefill half must still be REAL W4A4 NVFP4 for vLLM -- packed weights, an
    ...
    python tests/test_nvfp4lloyd21upcast.py
"""
import json
import os
import shutil
...
def test_prefill_is_an_upcast_of_the_decode_weight():
    ...
    check(f"{NAME}: layer class", isinstance(lin, NVFP4Lloyd21UpcastLinear))
    ...
    check("prefill DIFFERS from quantizing the master directly", d > 1e-6,
          f"max|diff| {d:.2e} -- otherwise this is just nvfp4lloyd21shared")


def test_upcast_source_lies_on_the_lloyd21_grid():
    """The tensor prefill upcasts FROM must be a genuine 3-bit weight.

    This is the storage claim: if the source were not on the Lloyd21 grid, the prefill
    ...
    # negatively-scaled blocks, and Lloyd21 is asymmetric, so those cannot lie on it.
...
def test_refresh_order_is_not_stale():
    ...
    would be an upcast of the PREVIOUS step's Lloyd21 weight -- wrong by one step, and
...
def test_export_is_real_nvfp4_w4a4_for_prefill():
    ...
    check("decode export is the Lloyd21-quantized weight", d2 < 1e-2, f"max|diff| {d2:.2e}")


def test_decode_grid_is_the_2bit_one():
    """The whole difference from nvfp4lloyd21upcast's 3-bit sibling is this LUT.

    A DECODE_GRID that failed to override would leave the layer on lloyd43 and produce a
    plausible 3-bit curve filed under a 2-bit name -- silent and unrecoverable after the
    fact, so it is asserted directly.
    """
    from quantizers.dual import NVFP4Lloyd43UpcastLinear
    m = build(NAME)
    lin = m.model.layers[0].mlp.gate_proj
    check("layer carries the 2-bit grid", lin._lloyd_grid.numel() == 4,
          f"{lin._lloyd_grid.numel()} levels")
    check("grid IS lloyd21, not lloyd43",
          torch.allclose(lin._lloyd_grid.cpu(), LLOYD21_SIGNED_2BIT, atol=1e-6))
    check("the 3-bit sibling still has 8 levels (untouched)",
          NVFP4Lloyd43UpcastLinear.DECODE_GRID.numel() == 8)
