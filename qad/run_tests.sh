...
#
#   ./run_tests.sh                      # every test (8 GPUs: test_checkpoint needs them)
#   ./run_tests.sh gsq_lloyd            # one file (tests/test_gsq_lloyd.py), 1 GPU
#   ./run_tests.sh gsq_lloyd quantizers # several
#   ./run_tests.sh -v gsq_lloyd         # full output instead of just PASS/FAIL lines
#
...
CMD='cd "$PWD"; export PYTHONPATH="$PWD"; failed=""'
for t in "${TESTS[@]}"; do
