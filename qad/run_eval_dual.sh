...
#
#   ./run_eval_dual.sh --steps 250,750,...  --model Qwen/Qwen3-0.6B
#   ./run_eval_dual.sh --steps 2450 --configs dual-shared,homo-a4   # subset
#
...
# (ample for GSM8K with --no-think), while eval_results_vllm/ is produced at 4096
# because AIME/MATH need it. A truncated answer scores 0, so the cap is part of the
# measurement. Everything in eval_results_dual/ shares one cap and is internally
...
if [ -z "$SLURM_JOB_ID" ]; then
    SELF="$(realpath "$0")"; ROOT="$(dirname "$SELF")/.."
...
if command -v scontrol &>/dev/null; then
    ...
    export SCRIPT_DIR HF_CACHE LM_EVAL_OVERLAY EV_STEPS EV_MODEL EV_CONFIGS EV_LIMIT EV_BS
...
exec python "$SCRIPT_DIR/eval_transformers.py" \
    --model "$EV_MODEL" \
    --quantizer "$QUANT" \
    --run-name "$RUN_NAME" \
    --ckpt-dir "$SCRIPT_DIR/checkpoints" \
    --iter "$STEP" \
    --tasks gsm8k \
    ${EXTRA:+$EXTRA} \
    --tag-suffix="-$LABEL" \
    ...
    --output-dir "$SCRIPT_DIR/eval_results_dual"
