...
def main() -> None:
    ...
    for prompt in (PROBE_PROMPTS[:args.limit] if args.limit else PROBE_PROMPTS):
        ...
        if r.status_code != 200:
            ...
            continue
        # A 200 does NOT guarantee a completion: when the decode engine dies mid
        # request vLLM can answer 200 with an error body, and indexing ["choices"]
        # blind turns that into a KeyError that destroys the run's evidence.
        body = r.json()
        if "choices" not in body:
            failures += 1
            print(f"[probe] 200 but no choices after {dt:.1f}s: {str(body)[:600]}",
                  flush=True)
            rows.append({"prompt": prompt, "error": f"no choices: {str(body)[:600]}"})
            continue
        completion = body["choices"][0]["text"]
